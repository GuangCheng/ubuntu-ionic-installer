...
NODE_X64="https://nodejs.org/download/release/v6.9.4/node-v6.9.4-linux-x64.tar.gz"
NODE_X86="https://nodejs.org/download/release/v6.9.4/node-v6.9.4-linux-x86.tar.gz"
...
if [ "$LINUX_ARCH" == "x86_64" ]; then
    ...
    cd "$INSTALL_PATH" && mv "node-v6.9.4-linux-x64" "node"
    ...
    cd "$INSTALL_PATH" && mv "android-sdk-linux" "android-sdk"
    cd "$INSTALL_PATH" && mv "node-v6.9.4-linux-x86" "node"
    cd "$INSTALL_PATH" && mv "gradle-2.9" "gradle"
